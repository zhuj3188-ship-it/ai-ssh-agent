...
async def ws_chat(ws: WebSocket):
    ...
    try:
        while True:
            ...
            result = await brain.chat(msg.get("message", ""), provider_name=msg.get("provider"), user_id=user_id)
            await ws.send_text(json.dumps({"type": "reply", "content": result["reply"],
                "tokens_in": result.get("tokens_in", 0), "tokens_out": result.get("tokens_out", 0),
                "cost": result.get("cost", 0), "provider": result.get("provider", ""), "model": result.get("model", "")}))
