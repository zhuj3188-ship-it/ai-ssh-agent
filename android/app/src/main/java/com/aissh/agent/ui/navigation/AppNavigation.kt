...
import com.aissh.agent.ui.theme.AiSshTheme
...
sealed class Screen(val route: String, val labelRes: Int, val icon: androidx.compose.ui.graphics.vector.ImageVector) {
    ...
    object Terminal : Screen("terminal", R.string.terminal, Icons.Default.Terminal)
...
fun AppNavigation() {
    ...
    AiSshTheme(darkTheme = settingsState.darkMode) {
        ...
        val screens = listOf(Screen.Chat, Screen.Terminal, Screen.Quota, Screen.Settings)
        val navBackStack by navController.currentBackStackEntryAsState()
        val currentRoute = navBackStack?.destination?.route
        val cs = MaterialTheme.colorScheme

        Scaffold(bottomBar = {
            NavigationBar(containerColor = cs.surface, tonalElevation = 0.5.dp) {
            ...
            NavHost(navController, startDestination = Screen.Chat.route, Modifier.padding(padding)) {
                ...
                composable(Screen.Terminal.route) { ServersScreen() }
